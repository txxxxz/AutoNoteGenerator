...
**2) 后端（Agent服务层）**
- 技术：LangGraph（流程编排）、LangChain（RAG与模型接口）。
- 框架：Streamlit（集成前后端逻辑，简化部署流程）。
- 功能：负责PPT解析、OCR识别、分块检索、内容生成与回贴。

**3) 数据库（RAG存储层）**
- 功能：将PPT内容处理为结构化的RAG知识库，作为AI学习助手的"课程记忆"，支持后续的深度学习与智能问答功能。
- 技术组件：
  - **FAISS**：用于构建向量索引，实现高效的语义检索；
  - **SQLite/PostgreSQL**：存储课程元数据、章节结构、用户配置等结构化信息；
  - **本地文件系统**：保存PPT截图、图表、公式等多媒体资源文件。
