...
const HistoryBar = ({ items, onRevert }: HistoryBarProps) => (
  <section className="panel-section" aria-labelledby="history-heading">
    ...
    <ul className="history-list">
      {items.map((item) => {
        const languageLabel = languageLabels[item.language ?? 'zh'] ?? '中文';
        return (
          <li key={item.id}>
            <div>
              <strong>
                {detailLabels[item.detailLevel] ?? item.detailLevel} × {expressionLabels[item.difficulty] ?? item.difficulty} ｜ {languageLabel}
              </strong>
